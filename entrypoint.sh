...
- \`docs: update README\`
      ...
      EXISTING_COMMENT=$(curl -s -H "Authorization: Bearer $GITHUB_TOKEN" \
        ...
        jq -r "[.[] | select(.body | contains(\"$COMMENT_MARKER\")) | select(.user.login == \"github-actions\")] | first | .id")
...
Your PR title follows the [Conventional Commits](https://www.conventionalcommits.org/) specification. Great job! 🎉"
      ...
      EXISTING_COMMENT=$(gh api "/repos/$GITHUB_REPOSITORY/issues/$PR_NUMBER/comments" --jq "[.[] | select(.body | contains(\"$COMMENT_MARKER\")) | select(.user.login == \"github-actions\")] | first | .id")
